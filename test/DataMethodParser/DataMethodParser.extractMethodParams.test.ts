...
@suite('Data Method Extract Method Parameters from string (extractMethodParams)')
class DataMethodParserUnitTest extends DataMethodParser {
   ...
   @test "extract 0 Params from string" () {
    ...
    @test "extract an optional completion handler ((_ String?) -> Void)? = nil with default value from parameters" () {
    ...
    @test "extract an optional completion handler ((_ String?) -> Void)? = nil  with default value from parameters followed by additional simple parameter" () {
    ...
    @test "extract parameter with a type which uses !" () {
        const data = this.mockData({params: ['person name: ((_ String!) -> Void)? = nil ' , 'surname: String']})
        const { remainingString } = this.extractMethodInformationFromString(data.join('\n'))
        const { params } = this.extractMethodParams(remainingString)
        expect(params.length).to.equal(2)
    }
